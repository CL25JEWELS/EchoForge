...
export const PadGrid: React.FC<PadGridProps> = React.memo(
  ({ pads, padStates, onPadTrigger, onPadStop, columns = 4, className = '' }) => {
    // ⚡ Bolt: Memoize the style object to prevent unnecessary object reference changes.
    // This ensures that React doesn't have to perform style updates if the grid
    // columns haven't changed, even when the component re-renders due to pad states.
