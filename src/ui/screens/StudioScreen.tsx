...
export const StudioScreen: React.FC<StudioScreenProps> = ({ app, className = '' }) => {
  ...
  const audioEngine = React.useMemo(() => app.getAudioEngine(), [app]);
  const projectManager = React.useMemo(() => app.getProjectManager(), [app]);
  const soundPackManager = React.useMemo(() => app.getSoundPackManager(), [app]);
  ...
  const soundPacks = React.useMemo(() => soundPackManager.getAllSoundPacks(), [soundPackManager]);
