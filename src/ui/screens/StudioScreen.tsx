...
export const StudioScreen: React.FC<StudioScreenProps> = ({ app, className = '' }) => {
  ...
  // ⚡ Bolt: Stabilize service instances to ensure they don't trigger re-renders
  // or dependency changes when passed to hooks or child components.
  ...
  const handleTempoChange = useCallback(
    (newTempo: Partial<TempoConfig>) => {
      setTempo((prev) => {
        const updatedTempo = { ...prev, ...newTempo };
  ...
  const handleSoundSelect = useCallback(
    ...
    [audioEngine, handlePadConfigChange, pads, soundPackManager]
  );
