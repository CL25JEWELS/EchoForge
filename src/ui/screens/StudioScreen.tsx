...
export const StudioScreen: React.FC<StudioScreenProps> = ({ app, className = '' }) => {
  ...
  // ⚡ Bolt: Stabilize service instances with useMemo.
  // This ensures stable references for hook dependencies and memoized child components.
  const audioEngine = useMemo(() => app.getAudioEngine(), [app]);
  const projectManager = useMemo(() => app.getProjectManager(), [app]);
  const soundPackManager = useMemo(() => app.getSoundPackManager(), [app]);
  ...
  // ⚡ Bolt: Memoize transport and mixer callbacks to prevent unnecessary re-renders
  // of PlaybackControls, which is now wrapped in React.memo.
  ...
  // ⚡ Bolt: Memoize sound selection callback.
  const handleSoundSelect = useCallback(
    ...
    [pads, handlePadConfigChange, soundPackManager, audioEngine]
  );

  // ⚡ Bolt: Memoize soundPacks to prevent unnecessary re-renders of SoundBrowser.
